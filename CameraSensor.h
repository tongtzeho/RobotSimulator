...
class CameraSensor : public CE::Camera
{
	...
	bool activated;
public:
	CameraSensor(const unsigned texWidth, const unsigned texHeight, const CE::Transform &transform = CE::Transform(), CE::Script **const scr = nullptr, const bool isPspt = true, const float nr = 1, const float fr = 100, const float fOrW = 0.25f*CE::Pi, const float depthFar = 50); // 透视摄像机的宽高比或正交摄像机的高可以根据其他数值得出
	virtual ~CameraSensor();
	virtual void PreRender(std::vector<GameObject*> __renderLists[], void *const param = nullptr);
